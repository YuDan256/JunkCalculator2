...
v2.4.4.0 版本进行了一次深度的架构重构与标准库大清洗。彻底解决了模块隔离与引用回写的历史遗留问题，大幅提升了 CAS 引擎的推导能力，并实现了容器 API 的全面现代化。

### 架构级重构：模块隔离与引用穿透
- **真正安全的命名空间模块系统**：重构了 `import` 机制，将脚本作为独立函数块编译并压入虚拟全局状态。执行完毕后通过 `OP_BUILD_NAMESPACE` 极速打包，实现了媲美 Node.js 的 100% 词法隔离，彻底杜绝全局环境污染。
- **统一的底层引用机制**：废除旧版延迟回写指令，引入全新的 `OP_PASS_REFS`。在编译期维护独立引用追踪表，运行时通过调用帧直接解引用，实现对外部物理内存的绝对 O(1) 实时穿透修改，修复了异常中断导致的状态丢失问题。
- **正交化的 Const 修饰符**：将 `const` 降级为赋值节点上的布尔标志，完美解锁 `const local x`、`for (const local x in ...)` 等组合语法。编译器统一接管写保护，运行时拒绝对常量的二次赋值。

### 数据结构：容器代数与性能防御
- **字典的聚合与剔除**：重载 `+` 运算符实现字典的“右向覆盖合并”；重载 `-` 运算符实现“纯函数式键剔除”（右操作数支持 Set/List/Dict），底层仅触发一次深拷贝后批量擦除键值。
- **异或与位运算重构**：新增 `^^` 及其复合赋值 `^^=`。对于整数执行极速按位异或，对于 `Set` 类型执行对称差（Symmetric Difference）。
- **字符串与 GC 护城河**：引入字符串预计算长度与 `reserve` 的 Fast Path，拼接性能提升一个数量级。全面排查标准库，在所有容器分配的危险区强制包裹 `GcObjGuard`，防止新生对象被 GC 误杀。

### 数学引擎：CAS 进化与 API 现代化
- **线性代数 API 现代化**：全面启用多重返回与自描述字典。例如 `[Q, R] = qr(A)` 完美解构；`factor(n)` 返回 `{质数: 指数}` 字典；`lsolve` 返回包含状态、基底、残差等字段的字典，并移除了冗余的 `linfo`。
- **极限引擎的终极补全**：Gruntz 算法增加方向参数以支持单侧逼近；通过变量替换完美支持 $x \to -\infty$；修复首项系数符号判定；将原有的 `oo` 显示统一替换为符合 IEEE 标准的 `inf`。
- **积分化简器的智能路由**：大幅提升“特定有理分式启发式策略”在引擎内部的打分权重。命中特定模式时直接走专有算法短路执行，大幅削减了复杂超越函数积分导致的 AST 树膨胀规模。
