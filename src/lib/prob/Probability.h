...
namespace jc {
    ...
    namespace prob {
        ...
        inline double lngamma(double x) {
            if (x <= 0 && x == std::floor(x))
                jc2::throw_error(jc2::ErrorType::MathError, "lgamma undefined at non-positive integers.");
            if (x <= 0) {
                double sinpx = std::sin(PI_ * x);
                if (std::abs(sinpx) < 1e-300)
                    jc2::throw_error(jc2::ErrorType::MathError, "lgamma pole.");
        ...
        inline double tgamma(double x) {
            if (x <= 0 && x == std::floor(x))
                jc2::throw_error(jc2::ErrorType::MathError, "Gamma undefined at non-positive integers.");
        ...
        inline double normalInvStd(double p) {
            if (p <= 0 || p >= 1) jc2::throw_error(jc2::ErrorType::MathError, "p must be in (0,1).");
    ...
    public:
        ...
        static Distribution normal(double mu = 0, double sigma = 1) {
            if (sigma <= 0) jc2::throw_error(jc2::ErrorType::MathError, "sigma must be positive.");
            return { Type::NORMAL, {mu, sigma} };
        }
        static Distribution studentT(double df) {
            if (df <= 0) jc2::throw_error(jc2::ErrorType::MathError, "df must be positive.");
        ...
        static Distribution fDist(double d1, double d2) {
            if (d1 <= 0 || d2 <= 0) jc2::throw_error(jc2::ErrorType::MathError, "df must be positive.");
            return { Type::F_DIST, {d1, d2} };
        }
        static Distribution exponential(double lam) {
            if (lam <= 0) jc2::throw_error(jc2::ErrorType::MathError, "lambda must be positive.");
            return { Type::EXPONENTIAL, {lam} };
        }
        static Distribution gammaDist(double shape, double rate) {
            if (shape <= 0 || rate <= 0) jc2::throw_error(jc2::ErrorType::MathError, "shape/rate must be positive.");
            return { Type::GAMMA_DIST, {shape, rate} };
        }
        static Distribution betaDist(double a, double b) {
            if (a <= 0 || b <= 0) jc2::throw_error(jc2::ErrorType::MathError, "alpha/beta must be positive.");
            return { Type::BETA_DIST, {a, b} };
        }
        static Distribution uniformDist(double a, double b) {
            if (a >= b) jc2::throw_error(jc2::ErrorType::MathError, "a must be less than b.");
            return { Type::UNIFORM_DIST, {a, b} };
        }
        static Distribution binomial(int n, double p) {
            if (n < 0) jc2::throw_error(jc2::ErrorType::MathError, "n must be non-negative.");
            if (p < 0 || p > 1) jc2::throw_error(jc2::ErrorType::MathError, "p must be in [0,1].");
            return { Type::BINOMIAL, {static_cast<double>(n), p} };
        }
        static Distribution poisson(double lam) {
            if (lam < 0) jc2::throw_error(jc2::ErrorType::MathError, "lambda must be non-negative.");
        ...
        double quantile(double p) const {
            ...
            case Type::BINOMIAL: case Type::POISSON: case Type::GEOMETRIC: {
                ...
                while (cdf(static_cast<double>(k)) < p) {
                    k++;
                    if (k > 1000000) jc2::throw_error(jc2::ErrorType::MathError, "quantile search exceeded limit.");
                }
        ...
        double distMean() const {
            ...
            case Type::STUDENT_T: { double df = params[0]; if (df <= 1) jc2::throw_error(jc2::ErrorType::MathError, "Mean undefined for df<=1."); return 0; }
            case Type::CHI_SQUARED:  return params[0];
            case Type::F_DIST: { double d2 = params[1]; if (d2 <= 2) jc2::throw_error(jc2::ErrorType::MathError, "Mean undefined for d2<=2."); return d2 / (d2 - 2); }
        ...
        double distVar() const {
            ...
            case Type::STUDENT_T: { double df = params[0]; if (df <= 2) jc2::throw_error(jc2::ErrorType::MathError, "Variance undefined for df<=2."); return df / (df - 2); }
            case Type::CHI_SQUARED:  return 2.0 * params[0];
            case Type::F_DIST: {
                double d1 = params[0], d2 = params[1];
                if (d2 <= 4) jc2::throw_error(jc2::ErrorType::MathError, "Variance undefined for d2<=4.");
    ...
    inline TestResult ttest1(const std::vector<double>& data, double mu0 = 0) {
        ...
        if (n < 2) jc2::throw_error(jc2::ErrorType::MathError, "t-test requires >= 2 data points.");
        double s = 0, sq = 0;
        for (double x : data) { s += x; sq += x * x; }
        double mean = s / n, var = (sq - s * s / n) / (n - 1);
        double se = std::sqrt(var / n);
        if (se < 1e-30) jc2::throw_error(jc2::ErrorType::MathError, "Zero variance.");
    ...
    inline TestResult ttest2ind(const std::vector<double>& d1, const std::vector<double>& d2) {
        ...
        if (n1 < 2 || n2 < 2) jc2::throw_error(jc2::ErrorType::MathError, "t-test requires >= 2 per group.");
    ...
    inline TestResult ttestPaired(const std::vector<double>& d1, const std::vector<double>& d2) {
        if (d1.size() != d2.size()) jc2::throw_error(jc2::ErrorType::MathError, "Paired test needs equal lengths.");
    ...
    inline TestResult chi2test(const std::vector<double>& obs, const std::vector<double>& exp) {
        if (obs.size() != exp.size()) jc2::throw_error(jc2::ErrorType::MathError, "obs/exp size mismatch.");
        if (obs.size() < 2) jc2::throw_error(jc2::ErrorType::MathError, "Need >= 2 categories.");
        double stat = 0;
        for (size_t i = 0; i < obs.size(); ++i) {
            if (exp[i] <= 0) jc2::throw_error(jc2::ErrorType::MathError, "Expected must be positive.");
