...
- **软类型匹配**：catch 模式如 `catch { {type: "TypeError", message} => ... }` 解构异常的 `type` 字段；不匹配则跳过该分支并重新抛出。
...
### 矩阵 — 列主序统一
- **统一矩阵模型**：矩阵是唯一类型，采用列主序语义——`A[i]` 返回第 i 列、迭代逐列、解包逐列。
- **去除向量优待**：行向量（1×N）与列向量（N×1）不再有特殊待遇；1×1 矩阵按矩阵乘法运算（显式降维走 `.item()`）。
- **列表与矩阵模式分离**：`@[...]` 解构列表，`[...]` 解构矩阵——不再有歧义。
- **严格往返**：`toList`/`toMatrix` 去除向量展平特例，成为精确互逆。
